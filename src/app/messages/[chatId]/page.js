...
export default function ChatPage({ params }) {
  ...
  // ── Route states ─────────────────────────────────────────────────────────
  if (authLoading || !user || loading) return <div className="min-h-screen bg-zinc-950 flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-rose-500" /></div>;
  return <ComingSoon />;

  return chatData ? <ChatView chatData={chatData} friendProfile={friendProfile} messages={messages} groupedMessages={groupedMessages} user={user} isSeen={!chatData.unreadBy || !chatData.unreadBy.includes(chatData.participants.find((participant) => participant !== user.uid))} newMessage={newMessage} sending={sending} messagesEndRef={messagesEndRef} inputRef={inputRef} onBack={() => router.push("/messages")} onSend={handleSend} onMessageChange={(event) => setNewMessage(event.target.value)} formatTime={formatTime} formatDate={formatDate} /> : <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-zinc-400 p-4 gap-4"><p className="text-sm">Sohbet bulunamadı.</p><Link href="/messages" className="px-5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-xl text-white text-sm font-semibold hover:bg-zinc-800 transition">Geri Dön</Link></div>;
}
