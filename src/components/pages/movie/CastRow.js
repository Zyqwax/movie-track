...
export default function CastRow({ cast, t }) {
  ...
  return (
    <section className="space-y-4" aria-labelledby="movie-cast-heading">
      ...
      <div className="-mx-4 flex snap-x gap-2 overflow-x-auto px-4 pb-2 scrollbar-hide md:-mx-0 md:gap-3 md:px-0">
        {actors.map((actor) => (
          <article key={`${actor.name}-${actor.character}`} className="relative aspect-[3/4] w-36 shrink-0 snap-start overflow-hidden rounded-[var(--radius-sm)] border border-border bg-surface-2 md:w-40">
            {actor.profilePath ? (
              <Image src={tmdbImage(actor.profilePath)} alt={actor.name} fill sizes="(min-width: 768px) 160px, 144px" className="object-cover" unoptimized />
