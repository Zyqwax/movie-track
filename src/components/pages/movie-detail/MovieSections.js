...
export function MovieCast({ cast, t }) {
  ...
  return <div className="mb-8"><h3 className="text-lg font-semibold text-white mb-3">{t("movie.cast")}</h3><div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">{cast.map((actor) => <div key={actor.name} className="flex flex-col items-center shrink-0 w-20"><div className="w-16 h-16 rounded-full bg-zinc-800 overflow-hidden mb-2 relative">{actor.profilePath && <Image src={`https://image.tmdb.org/t/p/w185${actor.profilePath}`} alt={actor.name} fill className="object-cover" unoptimized />}</div><span className="text-xs text-zinc-300 text-center line-clamp-1">{actor.name}</span><span className="text-[10px] text-zinc-500 text-center line-clamp-1">{actor.character}</span></div>)}</div></div>;
}
