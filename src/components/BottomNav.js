...
import { Home, Search, User, MessageCircle, Activity, LogOut } from "lucide-react";
...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { name: "Mesajlar", href: "/messages", icon: MessageCircle, badge: hasUnread },
    { name: "Etkinlik", href: "/activity", icon: Activity },
    { name: "Profil", href: "/profile", icon: User },
  ];

  /* ─── Shared Sidebar Content (Desktop) ─────────────────────────── */
  ...
  return (
    <>
      ...
      {/* ── MOBILE: Fixed Bottom Tab Bar (<lg) ──────────────────────── */}
      <nav
        className="lg:hidden fixed bottom-0 left-0 right-0 z-50 flex items-end"
        style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
      >
        {/* Blur/glassmorphism background */}
        <div className="absolute inset-0 bg-zinc-950/90 backdrop-blur-xl border-t border-zinc-800/70" />

        <div className="relative w-full flex items-center justify-around px-1 py-2">
          {navItems.map((item) => {
            const isActive =
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className="flex flex-col items-center justify-center flex-1 gap-1 py-1 select-none group outline-none"
                ...
                <div className="relative flex items-center justify-center">
                  ...
                  <span
                    className={clsx(
                      ...
                      isActive ? "scale-100 opacity-100 bg-rose-500/15" : "scale-75 opacity-0"
                    )}
                    style={{ padding: "10px 18px", margin: "-10px -18px" }}
                  />
                  ...
                  <item.icon
                    ...
                    className={clsx(
                      ...
                      isActive
                        ? "text-rose-400 stroke-[2.5] scale-110"
                        : "text-zinc-500 stroke-[1.8] group-active:scale-90"
                    )}
                  />
